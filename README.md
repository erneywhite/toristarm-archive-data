...
├── covers/                 # Обложки игр и кадры фильмов (webp) — те же, что на сайте
├── tools/
...
```jsonc
{
  ...
  "coverLocal":  "Filename.webp",       // файл обложки в covers/
...
- **Мягко** (предупреждение, сборку не валит): жанр вне словаря — вероятная опечатка;
  обложки или кадра, на который ссылается карточка, нет в `covers/`.
